...
if [[ -n "$INSTALLATION_DIR" ]]; then
  p9k_directory=${INSTALLATION_DIR:A}
else
  if [[ "${(%):-%N}" == '(eval)' ]]; then
    if [[ "$0" == '-antigen-load' ]] && [[ -r "${PWD}/powerlevel9k.zsh-theme" ]]; then
      ...
      print -P "%F{red}You must set INSTALLATION_DIR work from within an (eval).%f"
      return 1
    fi
  else
...
#     uppercase it. This is then prefixed with "" and suffixed
...
set_default WHITESPACE_BETWEEN_LEFT_SEGMENTS " "
left_prompt_segment() {
  local segment_name="${1}"
  local current_index=$2
  # Check if the segment should be joined with the previous one
  local joined
  segmentShouldBeJoined $current_index $last_left_element_index "$LEFT_PROMPT_ELEMENTS" && joined=true || joined=false
  ...
  local BACKGROUND_USER_VARIABLE=${(U)${segment_name}#prompt_}_BACKGROUND
  local BG_COLOR_MODIFIER=${(P)BACKGROUND_USER_VARIABLE}
  [[ -n $BG_COLOR_MODIFIER ]] && backgroundColor="$BG_COLOR_MODIFIER"

  # Overwrite given foreground-color by user defined variable for this segment.
  local FOREGROUND_USER_VARIABLE=${(U)${segment_name}#prompt_}_FOREGROUND
  ...
  if [[ $CURRENT_BG != 'NONE' ]] && ! isSameColor "${backgroundColor}" "$CURRENT_BG"; then
    ...
    if [[ $joined == false ]]; then
      # Middle segment
      echo -n "$(print_icon 'LEFT_SEGMENT_SEPARATOR')$WHITESPACE_BETWEEN_LEFT_SEGMENTS"
    fi
    ...
    if [[ $joined == false ]]; then
      echo -n "$(print_icon 'LEFT_SUBSEGMENT_SEPARATOR')$WHITESPACE_BETWEEN_LEFT_SEGMENTS"
    fi
  else
    # First segment
    echo -n "${background}$WHITESPACE_BETWEEN_LEFT_SEGMENTS"
  fi
  ...
  if [[ -n $6 ]]; then
    ...
    if [[ -n "$visual_identifier" ]]; then
      ...
      local visual_identifier_color_variable=${(U)${segment_name}#prompt_}_VISUAL_IDENTIFIER_COLOR
  ...
  echo -n "${WHITESPACE_BETWEEN_LEFT_SEGMENTS}"
...
set_default WHITESPACE_BETWEEN_RIGHT_SEGMENTS " "
right_prompt_segment() {
  ...
  # Check if the segment should be joined with the previous one
  local joined
  segmentShouldBeJoined $current_index $last_right_element_index "$RIGHT_PROMPT_ELEMENTS" && joined=true || joined=false
  ...
  if [[ "$CURRENT_RIGHT_BG" != "NONE" ]]; then
    ...
    echo -n "${WHITESPACE_BETWEEN_RIGHT_SEGMENTS}%f"
  fi
  ...
  [[ $joined == false ]] || [[ "$CURRENT_RIGHT_BG" == "NONE" ]] && echo -n "${WHITESPACE_BETWEEN_RIGHT_SEGMENTS}"
...
prompt_anaconda() {
  ...
  if ! [ -z "$_path" ]; then
    ...
    set_default ANACONDA_LEFT_DELIMITER "("
    set_default ANACONDA_RIGHT_DELIMITER ")"
    "$1_prompt_segment" "$0" "$2" "blue" "$DEFAULT_COLOR" "$ANACONDA_LEFT_DELIMITER$(basename $_path)$ANACONDA_RIGHT_DELIMITER" 'PYTHON_ICON'
  fi
}
...
set_default BACKGROUND_JOBS_VERBOSE true
set_default BACKGROUND_JOBS_VERBOSE_ALWAYS false
prompt_background_jobs() {
  ...
  if [[ background_jobs_number -gt 0 ]]; then
    ...
    if [[ "$BACKGROUND_JOBS_VERBOSE" == "true" ]] && ([[ "$background_jobs_number" -gt 1 ]] || [[ "$BACKGROUND_JOBS_VERBOSE_ALWAYS" == "true" ]]); then
...
prompt_newline() {
  ...
  [[ "$1" == "right" ]] && return
  newline=$'\n'
  lws=$WHITESPACE_BETWEEN_LEFT_SEGMENTS
  if [[ "$PROMPT_ON_NEWLINE" == true ]]; then
    newline="${newline}$(print_icon 'MULTILINE_NEWLINE_PROMPT_PREFIX')"
  fi
  WHITESPACE_BETWEEN_LEFT_SEGMENTS=
  "$1_prompt_segment" \
    "$0" \
    "$2" \
    "" "" "${newline}"
  CURRENT_BG='NONE'
  WHITESPACE_BETWEEN_LEFT_SEGMENTS=$lws
}

################################################################
# Segment that indicates usage level of current partition.
set_default DISK_USAGE_ONLY_WARNING false
set_default DISK_USAGE_WARNING_LEVEL 90
set_default DISK_USAGE_CRITICAL_LEVEL 95
prompt_disk_usage() {
  ...
  if [ "$disk_usage" -ge "$DISK_USAGE_WARNING_LEVEL" ]; then
    current_state='warning'
    if [ "$disk_usage" -ge "$DISK_USAGE_CRITICAL_LEVEL" ]; then
        current_state='critical'
    fi
  else
    if [[ "$DISK_USAGE_ONLY_WARNING" == true ]]; then
        current_state=''
        return
    fi
...
prompt_battery() {
  ...
  set_default BATTERY_LOW_THRESHOLD  10
  ...
  if [[ $OS =~ OSX && -f "${ROOT_PREFIX}"/usr/bin/pmset && -x "${ROOT_PREFIX}"/usr/bin/pmset ]]; then
    ...
    case $(echo $raw_data | awk -F ';' '{print $2}' | awk '{$1=$1};1') in
      ...
      'charging'|'finishing charge'|'AC attached')
        ...
        ;;
      'discharging')
        [[ $bat_percent -lt $BATTERY_LOW_THRESHOLD ]] && current_state="low" || current_state="disconnected"
        remain=" ($tstring)"
        ;;
      *)
  ...
  set_default BATTERY_VERBOSE true
  if [[ "$BATTERY_VERBOSE" == true ]]; then
  ...
  if [[ -n "$BATTERY_STAGES" ]]; then
    local segment=$(( 100.0 / (${#BATTERY_STAGES} - 1 ) ))
    if [[ $segment > 1 ]]; then
      local offset=$(( ($bat_percent / $segment) + 1 ))
      # check if the stages are in an array or a string
      [[ "${(t)BATTERY_STAGES}" =~ "array" ]] && BATTERY_ICON="$BATTERY_STAGES[$offset]" || BATTERY_ICON=${BATTERY_STAGES:$offset:1}
    fi
  fi
  # return if BATTERY_HIDE_ABOVE_THRESHOLD is set and the battery percentage is greater or equal
  if defined BATTERY_HIDE_ABOVE_THRESHOLD && [[ "${bat_percent}" -ge $BATTERY_HIDE_ABOVE_THRESHOLD ]]; then
    return
  fi

    # override the default color if we are using a color level array
    if [[ -n "$BATTERY_LEVEL_BACKGROUND" ]] && [[ "${(t)BATTERY_LEVEL_BACKGROUND}" =~ "array" ]]; then
      local segment=$(( 100.0 / (${#BATTERY_LEVEL_BACKGROUND} - 1 ) ))
      local offset=$(( ($bat_percent / $segment) + 1 ))
      "$1_prompt_segment" "$0_${current_state}" "$2" "${BATTERY_LEVEL_BACKGROUND[$offset]}" "${battery_states[$current_state]}" "${message}" "BATTERY_ICON"
    else
      # Draw the prompt_segment
...
prompt_public_ip() {
  ...
  set_default PUBLIC_IP_TIMEOUT "300"
  set_default PUBLIC_IP_NONE ""
  set_default PUBLIC_IP_FILE "/tmp/p9k_public_ip"
  set_default PUBLIC_IP_HOST "http://ident.me"
  defined PUBLIC_IP_METHODS || PUBLIC_IP_METHODS=(dig curl wget)
  ...
  if [[ -f $PUBLIC_IP_FILE ]]; then
    ...
    if [[ "$OS" == "OSX" ]]; then
      timediff=$(($(date +%s) - $(stat -f "%m" $PUBLIC_IP_FILE)))
    else
      timediff=$(($(date +%s) - $(date -r $PUBLIC_IP_FILE +%s)))
    fi
    [[ $timediff -gt $PUBLIC_IP_TIMEOUT ]] && refresh_ip=true
    # If tmp file is empty get a fresh IP
    [[ -z $(cat $PUBLIC_IP_FILE) ]] && refresh_ip=true
    [[ -n $PUBLIC_IP_NONE ]] && [[ $(cat $PUBLIC_IP_FILE) =~ "$PUBLIC_IP_NONE" ]] && refresh_ip=true
  else
    touch $PUBLIC_IP_FILE && refresh_ip=true
  fi

  # grab a fresh IP if needed
  local fresh_ip
  if [[ $refresh_ip == true && -w $PUBLIC_IP_FILE ]]; then
    for method in "${PUBLIC_IP_METHODS[@]}"; do
      case $method in
        'dig')
        ...
        'curl')
          ...
          ;;
        'wget')
            fresh_ip="$(wget -T 10 -qO- "$PUBLIC_IP_HOST" 2> /dev/null)"
          ;;
      esac
    ...
    [[ -n "${fresh_ip}" ]] && echo $fresh_ip >! $PUBLIC_IP_FILE || echo $PUBLIC_IP_NONE >! $PUBLIC_IP_FILE
  fi

  # read public IP saved to tmp file
  local public_ip="$(cat $PUBLIC_IP_FILE)"

  # Draw the prompt segment
  if [[ -n $public_ip ]]; then
    icon='PUBLIC_IP_ICON'
    # Check VPN is on if VPN interface is set
    if [[ -n $PUBLIC_IP_VPN_INTERFACE ]]; then
      local vpnIp="$(p9k::parseIp "${PUBLIC_IP_VPN_INTERFACE}" "${ROOT_PREFIX}")"
      if [[ -n "$vpnIp" ]]; then
        icon='VPN_ICON'
      fi
    fi
...
set_default ALWAYS_SHOW_CONTEXT false
set_default ALWAYS_SHOW_USER false
set_default CONTEXT_TEMPLATE "%n@%m"
prompt_context() {
  ...
  if [[ "$ALWAYS_SHOW_CONTEXT" == true ]] || [[ "$(whoami)" != "$DEFAULT_USER" ]] || [[ -n "$SSH_CLIENT" || -n "$SSH_TTY" ]]; then
      content="${CONTEXT_TEMPLATE}"
  elif [[ "$ALWAYS_SHOW_USER" == true ]]; then
      content="$(whoami)"
  else
      return
  fi
...
# User: user (who am I)
# Note that if $DEFAULT_USER is not set, this prompt segment will always print
set_default USER_TEMPLATE "%n"
prompt_user() {
  local current_state="DEFAULT"
  typeset -AH user_state
  if [[ "$ALWAYS_SHOW_USER" == true ]] || [[ "$(whoami)" != "$DEFAULT_USER" ]]; then
    if [[ $(print -P "%#") == '#' ]]; then
      user_state=(
        "STATE"               "ROOT"
...
prompt_custom() {
  ...
  local command="CUSTOM_${segment_name}"
...
prompt_command_execution_time() {
  set_default COMMAND_EXECUTION_TIME_THRESHOLD 3
  set_default COMMAND_EXECUTION_TIME_PRECISION 2
  ...
  if (( _P9K_COMMAND_DURATION > 3600 )); then
    ...
    # Convert to float
    if [[ "${COMMAND_EXECUTION_TIME_PRECISION}" == "0" ]]; then
      # If user does not want microseconds, then we need to convert
      # the duration to an integer.
      typeset -i humanReadableDuration
    else
      typeset -F ${COMMAND_EXECUTION_TIME_PRECISION} humanReadableDuration
    fi
    humanReadableDuration=$_P9K_COMMAND_DURATION
  fi

  if (( _P9K_COMMAND_DURATION >= COMMAND_EXECUTION_TIME_THRESHOLD )); then
...
#   * $1 Alignment: string - left|right
#   * $2 Index: integer
set_default DIR_PATH_SEPARATOR "/"
set_default HOME_FOLDER_ABBREVIATION "~"
set_default DIR_PATH_HIGHLIGHT_BOLD false
prompt_dir() {
  # using $PWD instead of "$(print -P '%~')" to allow use of DIR_PATH_ABSOLUTE
  local current_path=$PWD # WAS: local current_path="$(print -P '%~')"
  # check if the user wants to use absolute paths or "~" paths
  [[ ${(L)DIR_PATH_ABSOLUTE} != "true" ]] && current_path=${current_path/#$HOME/"~"}
  ...
  if [[ -n "$SHORTEN_DIR_LENGTH" || "$SHORTEN_STRATEGY" == "truncate_with_folder_marker" || "$SHORTEN_STRATEGY" == "truncate_to_last" || "$SHORTEN_STRATEGY" == "truncate_with_package_name" ]]; then
    set_default SHORTEN_DELIMITER "\u2026"
    # convert delimiter from unicode to literal character, so that we can get the correct length later
    local delim=$(echo -n $SHORTEN_DELIMITER)

    case "$SHORTEN_STRATEGY" in
      truncate_absolute_chars)
        if [ ${#current_path} -gt $(( $SHORTEN_DIR_LENGTH + ${#SHORTEN_DELIMITER} )) ]; then
          current_path=$SHORTEN_DELIMITER${current_path:(-SHORTEN_DIR_LENGTH)}
        fi
      ;;
      truncate_middle)
        # truncate characters from the middle of the path
        current_path=$(truncatePath $current_path $SHORTEN_DIR_LENGTH $SHORTEN_DELIMITER "middle")
      ;;
      truncate_from_right)
        # truncate characters from the right of the path
        current_path=$(truncatePath "$current_path" $SHORTEN_DIR_LENGTH $SHORTEN_DELIMITER)
      ;;
      truncate_absolute)
        # truncate all characters except the last SHORTEN_DIR_LENGTH characters
        if [ ${#current_path} -gt $(( $SHORTEN_DIR_LENGTH + ${#SHORTEN_DELIMITER} )) ]; then
          current_path=$SHORTEN_DELIMITER${current_path:(-SHORTEN_DIR_LENGTH)}
        fi
      ;;
      ...
      truncate_to_first_and_last)
        if (( ${#current_path} > 1 )) && (( ${SHORTEN_DIR_LENGTH} > 0 )); then
          threshhold=$(( ${SHORTEN_DIR_LENGTH} * 2))
          # if we are in "~", add it back into the paths array
          [[ $current_path == '~'* ]] && paths=("~" "${paths[@]}")
          if (( ${#paths} > $threshhold )); then
            local num=$(( ${#paths} - ${SHORTEN_DIR_LENGTH} ))
            # repace the middle elements
            for (( i=$SHORTEN_DIR_LENGTH; i<$num; i++ )); do
              paths[$i+1]=$SHORTEN_DELIMITER
            done
        ...
        if (( ${#paths} > 0 )); then # root and home are exceptions and won't have paths, so skip this
          ...
          for marked_folder in $(upsearch $SHORTEN_FOLDER_MARKER); do
            if [[ "$marked_folder" == "/" ]]; then
              ...
              trunc_path="${trunc_path%/}/$SHORTEN_DELIMITER/${marked_folder##*/}"
            fi
      ...
      truncate_with_package_name)
        ...
        [[ ${(L)DIR_PATH_ABSOLUTE} != "true" ]] && package_path=${package_path/$HOME/"~"}
        ...
        subdirectory_path=$(truncatePath "${trunc_path:${#${(S%%)package_path//$~zero/}}}" $SHORTEN_DIR_LENGTH $SHORTEN_DELIMITER)
        # Parse the 'name' from the package.json; if there are any problems, just
        # print the file path
        defined DIR_PACKAGE_FILES || DIR_PACKAGE_FILES=(package.json composer.json)

        local pkgFile="unknown"
        for file in "${DIR_PACKAGE_FILES[@]}"; do
      ...
      *)
        if [[ $current_path != "~" ]]; then
          current_path="$(print -P "%$((SHORTEN_DIR_LENGTH+1))(c:$SHORTEN_DELIMITER/:)%${SHORTEN_DIR_LENGTH}c")"
        fi
      ;;
    esac
  fi
  ...
  if [[ $state_path == '/etc'* ]]; then
    current_state='ETC'
  elif [[ "${DIR_SHOW_WRITABLE}" == true && ! -w "$PWD" ]]; then
  ...
  if [[ "${(L)DIR_PATH_HIGHLIGHT_BOLD}" == "true" ]]; then
    bld_on="%B"
    bld_off="%b"
  else
    bld_on=""
  ...
  local dir_state_user_foreground=DIR_${current_state}_FOREGROUND
  ...
  if [[ -n ${DIR_PATH_HIGHLIGHT_FOREGROUND} ]]; then
    # it the path is "/" or "~"
    if [[ $path_opt == "/" || $path_opt == "~" ]]; then
      current_path="${bld_on}%F{$DIR_PATH_HIGHLIGHT_FOREGROUND}${current_path}${bld_off}"
      ...
      if [[ $dir_name != $base_name ]]; then
        current_path="${dir_name}/${bld_on}%F{$DIR_PATH_HIGHLIGHT_FOREGROUND}${base_name}${bld_off}"
      else
        current_path="${bld_on}%F{$DIR_PATH_HIGHLIGHT_FOREGROUND}${base_name}${bld_off}"
      fi
    fi
  else # no coloring
    # it the path is "/" or "~"
  ...
  if [[ "${DIR_OMIT_FIRST_CHARACTER}" == "true" && $path_opt != "/" && $path_opt != "~" ]]; then
    current_path="${current_path[2,-1]}"
  fi

  # check if the user wants the separator colored.
  if [[ -n ${DIR_PATH_SEPARATOR_FOREGROUND} && $path_opt != "/" ]]; then
    # because this contains color changing codes, it is easier to set a variable for what should be replaced
    local repl="%F{$DIR_PATH_SEPARATOR_FOREGROUND}/%F{$dir_state_foreground}"
    # escape the / with a \
    current_path=${current_path//\//$repl}
  fi

  if [[ "${DIR_PATH_SEPARATOR}" != "/" && $path_opt != "/" ]]; then
    current_path=${current_path//\//$DIR_PATH_SEPARATOR}
  fi

  if [[ "${HOME_FOLDER_ABBREVIATION}" != "~" && ! "${(L)DIR_OMIT_FIRST_CHARACTER}" == "true" ]]; then
    # use :s to only replace the first occurance
    current_path=${current_path:s/~/$HOME_FOLDER_ABBREVIATION}
  fi
...
prompt_ip() {
  local ROOT_PREFIX="${4}"
  local ip=$(p9k::parseIp "${IP_INTERFACE}" "${ROOT_PREFIX}")
...
set_default VPN_IP_INTERFACE "tun"
# prompt if vpn active
prompt_vpn_ip() {
  local ROOT_PREFIX="${4}"
  local ip=$(p9k::parseIp "${VPN_IP_INTERFACE}" "${ROOT_PREFIX}")
...
set_default RBENV_PROMPT_ALWAYS_SHOW false
prompt_rbenv() {
  if [[ -n "$RBENV_VERSION" ]]; then
    ...
    if [[ "${rbenv_version_name}" != "${rbenv_global}" || "${RBENV_PROMPT_ALWAYS_SHOW}" == "true" ]]; then
...
prompt_chruby() {
  ...
  set_default CHRUBY_SHOW_VERSION true
  set_default CHRUBY_SHOW_ENGINE true
  local chruby_label=""

  if [[ "$CHRUBY_SHOW_ENGINE" == true ]]; then
    chruby_label+="$RUBY_ENGINE "
  fi
  if [[ "$CHRUBY_SHOW_VERSION" == true ]]; then
...
#
set_default STATUS_CROSS false
set_default STATUS_OK true
set_default STATUS_SHOW_PIPESTATUS true
set_default STATUS_HIDE_SIGNAME false
# old options, retro compatibility
set_default STATUS_VERBOSE true
set_default STATUS_OK_IN_NON_VERBOSE false

exit_code_or_status() {
  local ec=$1
  if [[ "$STATUS_HIDE_SIGNAME" = true ]]; then
    echo "$ec"
  elif (( ec <= 128 )); then
    echo "$ec"
  else
...
prompt_status() {
  ...
  if [[ $STATUS_SHOW_PIPESTATUS == true ]]; then
    if (( $#RETVALS > 1 )); then
  ...
  if (( ec_sum > 0 )); then
    if [[ "$STATUS_CROSS" == false && "$STATUS_VERBOSE" == true ]]; then
      "$1_prompt_segment" "$0_ERROR" "$2" "red" "yellow1" "$ec_text" 'CARRIAGE_RETURN_ICON'
    else
      "$1_prompt_segment" "$0_ERROR" "$2" "$DEFAULT_COLOR" "red" "" 'FAIL_ICON'
    fi
  elif [[ "$STATUS_OK" == true ]] && [[ "$STATUS_VERBOSE" == true || "$STATUS_OK_IN_NON_VERBOSE" == true ]]; then
...
# System time
prompt_time() {
  set_default TIME_FORMAT "%D{%H:%M:%S}"
...
# System date
prompt_date() {
  set_default DATE_FORMAT "%D{%d.%m.%y}"
...
# version control
set_default VCS_ACTIONFORMAT_FOREGROUND "red"
# Default: Just display the first 8 characters of our changeset-ID.
set_default VCS_INTERNAL_HASH_LENGTH "8"
powerlevel9k_vcs_init() {
  if [[ -n "$CHANGESET_HASH_LENGTH" ]]; then
    VCS_INTERNAL_HASH_LENGTH="$CHANGESET_HASH_LENGTH"
  fi
  ...
  if [[ "$SHOW_CHANGESET" == true ]]; then
    VCS_CHANGESET_PREFIX="$(print_icon 'VCS_COMMIT_ICON')%0.$VCS_INTERNAL_HASH_LENGTH""i "
  fi
  ...
  defined VCS_GIT_HOOKS || VCS_GIT_HOOKS=(vcs-detect-changes git-untracked git-aheadbehind git-stash git-remotebranch git-tagname)
  zstyle ':vcs_info:git*+set-message:*' hooks $VCS_GIT_HOOKS
  defined VCS_HG_HOOKS || VCS_HG_HOOKS=(vcs-detect-changes)
  zstyle ':vcs_info:hg*+set-message:*' hooks $VCS_HG_HOOKS
  defined VCS_SVN_HOOKS || VCS_SVN_HOOKS=(vcs-detect-changes svn-detect-changes)
  zstyle ':vcs_info:svn*+set-message:*' hooks $VCS_SVN_HOOKS
  ...
  zstyle ':vcs_info:svn*:*' actionformats "$VCS_CHANGESET_PREFIX%c%u %F{${VCS_ACTIONFORMAT_FOREGROUND}}| %a%f"
...
set_default VI_INSERT_MODE_STRING "INSERT"
set_default VI_COMMAND_MODE_STRING "NORMAL"
prompt_vi_mode() {
  case ${KEYMAP} in
    vicmd)
      "$1_prompt_segment" "$0_NORMAL" "$2" "$DEFAULT_COLOR" "white" "$VI_COMMAND_MODE_STRING"
    ;;
    main|viins|*)
      if [[ -z $VI_INSERT_MODE_STRING ]]; then return; fi
      "$1_prompt_segment" "$0_INSERT" "$2" "$DEFAULT_COLOR" "blue" "$VI_INSERT_MODE_STRING"
    ;;
  esac
}
...
set_default PYENV_PROMPT_ALWAYS_SHOW false
prompt_pyenv() {
  if [[ -n "$PYENV_VERSION" ]]; then
    ...
    if [[ "${pyenv_version_name}" != "${pyenv_global}" || "${PYENV_PROMPT_ALWAYS_SHOW}" == "true" ]]; then
...
build_left_prompt() {
  local index=1
  local element
  for element in "${LEFT_PROMPT_ELEMENTS[@]}"; do
...
build_right_prompt() {
  local index=1
  local element
  for element in "${RIGHT_PROMPT_ELEMENTS[@]}"; do
    # Remove joined information in direct calls
    element=${element%_joined}
...
set_default PROMPT_ADD_NEWLINE false
...
$(print_icon 'MULTILINE_LAST_PROMPT_PREFIX')'
    if [[ "$RPROMPT_ON_NEWLINE" != true ]]; then
...
'
  ...
  if [[ $PROMPT_ADD_NEWLINE == true ]]; then
    NEWLINES=""
    repeat ${PROMPT_ADD_NEWLINE_COUNT:-1} { NEWLINES+=$NEWLINE }
...
set_default IGNORE_TERM_COLORS false
set_default IGNORE_TERM_LANG false
...
prompt_powerlevel9k_setup() {
  ...
  if [[ $IGNORE_TERM_LANG == false ]]; then
      local term_lang
      term_lang=$(echo $LANG)
      if [[ $term_lang == 'C' ]]; then
  ...
  defined LEFT_PROMPT_ELEMENTS || LEFT_PROMPT_ELEMENTS=(context dir vcs)
  defined RIGHT_PROMPT_ELEMENTS || RIGHT_PROMPT_ELEMENTS=(status root_indicator background_jobs history time)
