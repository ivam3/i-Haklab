...
set_default VCS_SHOW_SUBMODULE_DIRTY false
function +vi-git-untracked() {
  ...
  if [[ -z $untrackedFiles && "$VCS_SHOW_SUBMODULE_DIRTY" == "true" ]]; then
...
function +vi-git-remotebranch() {
    ...
    if [[ -n "$VCS_SHORTEN_LENGTH" ]] && [[ -n "$VCS_SHORTEN_MIN_LENGTH" ]]; then
     set_default VCS_SHORTEN_DELIMITER $'\U2026'

     if [ ${#hook_com[branch]} -gt ${VCS_SHORTEN_MIN_LENGTH} ] && [ ${#hook_com[branch]} -gt ${VCS_SHORTEN_LENGTH} ]; then
       case "$VCS_SHORTEN_STRATEGY" in
         truncate_middle)
           hook_com[branch]="${branch_name:0:$VCS_SHORTEN_LENGTH}${VCS_SHORTEN_DELIMITER}${branch_name: -$VCS_SHORTEN_LENGTH}"
         ;;
         truncate_from_right)
           hook_com[branch]="${branch_name:0:$VCS_SHORTEN_LENGTH}${VCS_SHORTEN_DELIMITER}"
         ;;
       esac
     fi
    fi
...
set_default VCS_HIDE_TAGS false
function +vi-git-tagname() {
    if [[ "$VCS_HIDE_TAGS" == "false" ]]; then
        ...
        if [[ -n "${tag}" ]] ; then
            ...
            if [[ -z $(command git symbolic-ref HEAD 2>/dev/null) ]]; then
                ...
                # string.
                local revision
                revision=$(command git rev-list -n 1 --abbrev-commit --abbrev=${VCS_INTERNAL_HASH_LENGTH} HEAD)
