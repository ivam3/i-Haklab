...
segment_in_use() {
    local key=$1
    [[ -n "${LEFT_PROMPT_ELEMENTS[(r)${key}]}" ||
     -n "${LEFT_PROMPT_ELEMENTS[(r)${key}_joined]}" ||
     -n "${RIGHT_PROMPT_ELEMENTS[(r)${key}]}" ||
     -n "${RIGHT_PROMPT_ELEMENTS[(r)${key}_joined]}" ]]
}
...
function truncatePathFromRight() {
  local delim_len=${#SHORTEN_DELIMITER:-1}
  echo $1 | sed $SED_EXTENDED_REGEX_PARAMETER \
 "s@(([^/]{$((SHORTEN_DIR_LENGTH))})([^/]{$delim_len}))[^/]+/@\2$SHORTEN_DELIMITER/@g"
}
