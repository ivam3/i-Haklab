...
function sane_environment_check
  ...
  set -l result (printf 'a\nb\n' | cmd head -n 1)
    and test "$result" = 'a'
    or abort (command -v head)" is not a sane 'head' implementation"
  ...
  set -l result (printf '1.2.3\n2.2.4\n1.2.4\n' | cmd sort -r -n -t '.' -k 1,1 -k 2,2 -k 3,3 -k 4,4)
    ...
    or abort (command -v sort)" is not a sane 'sort' implementation"

  debug "Checking for a working AWK interpreter"
  cmd awk 'BEGIN{exit 42;}' < /dev/null 2> /dev/null
  if not test $status -eq 42
    abort (command -v awk)" does not look like an AWK interpreter."
  end
  ...
  if cmd git --version | cmd grep -i -q windows
    abort (command -v git)" is Git for Windows which is not supported."
  end
...
function assert_cmds
  ...
  for cmd in $cmds
    ...
    debug "Command '$cmd' is "(command -v $cmd)
  end
end
...
function which
  if type realpath > /dev/null 2> /dev/null
    realpath (command command -v $argv)
  else
    command readlink (command command -v $argv)
  end
end
